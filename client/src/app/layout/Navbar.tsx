...
import { useStore } from '../stores/store';
  ...
  export default function WithSubnavigation() {
    ...
    return (
      <Box>
        <Flex bg="linear-gradient(to right top, #d16ba5, #c777b9, #ba83ca, #aa8fd8, #9a9ae1, #8aa7ec, #79b3f4, #69bff8, #52cffe, #41dfff, #46eefa, #5ffbf1) "  color={useColorModeValue('gray.600', 'white')} minH={'60px'} py={{ base: 2 }} px={{ base: 4 }} borderBottom={1} borderStyle={'solid'} borderColor={useColorModeValue('gray.200', 'gray.900')} align={'center'}>
          <Flex
            flex={{ base: 1, md: 'auto' }}
            ml={{ base: -2 }}
          ...
          <Stack align={"center"} flex={{ base: 1, md: 0 }} justify={'flex-end'} direction={'row'} spacing={6}>
            ...
            <Link href={"/createActivity"} p={".5em 0"} fontSize={'xl'}  _hover={{   textDecoration: 'none'}}>
              <Button display={{ base: 'none', md: 'inline-flex' }} fontSize={'sm'} fontWeight={600} color={'white'} bg={"green.200"} _hover={{bg: 'green.300',}}>
                Create Activity
              </Button>
            </Link>
          </Stack>
