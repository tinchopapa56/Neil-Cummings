...
const ActivityDetails: React.FC<Props> = ({ACTDetails}) => {
    return (
        // <Text>{ACTDetails.title}</Text>
    ...
    //   <Center py={6}>
        <Box maxW={'320px'} w={'full'} bg={useColorModeValue('white', 'gray.900')} boxShadow={'2xl'} rounded={'lg'} p={6} textAlign={'center'}>
          ...
          />
          <Heading fontSize={'2xl'} fontFamily={'body'}> {ACTDetails.title}</Heading>
          ...
          <Stack mt={8} direction={'row'} spacing={4}>
            ...
            <Button flex={1} fontSize={'sm'} rounded={'full'} bg={'blue.400'} color={'white'} boxShadow={   '0px 1px 25px -5px rgb(66 153 225 / 48%), 0 10px 10px -5px rgb(66 153 225 / 43%)' } _hover={{ bg: 'blue.500', }} _focus={{ bg: 'blue.500', }}>
              Cancel
            </Button>
          </Stack>
